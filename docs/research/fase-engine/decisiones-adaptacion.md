...
4. **Contrato de salida a emular (verificar contra el export real de katago-onnx antes de cablear índices):** `value[b,3]`=[win,loss,noResult] logits (jugador al turno); `miscvalue[b,10]`→`scoreValue[4]`=[scoreMean, stdev_preSoftplus, lead, varTimeLeft] con `[0]·20=scoreMean, softplus([1])·20=stdev, [2]·20=lead`, y ×(1−noResultProb); `ownership[b,1,H,W]` pre-tanh. **`postProcessParams`** (mult=20, outputScale=1) NO están en el contrato ONNX → constante por-modelo; correctos para b18c384nbt; **verificar los 7 floats del header de humanv0** (parse ASCII del `.bin`, sin TF.js). Riesgo de doble-postproceso: confirmar que el grafo ONNX emite crudos (no ya-softmaxeado/tanh).
