...
8. **Flags del área para planos 18/19 — solo Task 10 los gatea (confirmado en Task 4).** `computeAreaMapV7KataGo(Into)` hardcodea `safeBigTerritories: true` **Y** `unsafeBigTerritories: true`, y su rama "unsafe" omite el guard `!BORDERS_NONPASSALIVE` que sí exigen las ramas "safe" — es decir, marca territorio de un color aunque su grupo haya sido matado por Benson, siempre que la región no toque al rival. KataGo, en `fillRowV7` (`nninputs.cpp`), computa el área de los planos 18/19 con flags concretos (ver `fuentes.md §1`); **verificar que coinciden con lo que hardcodea web-katrain.** OJO al gate: el **oráculo diferencial de Task 5** (nuestro encoder NCHW vs `featuresV7Fast` NHWC) **NO** detecta una divergencia de flags respecto a KataGo, porque ambos encoders llaman a la MISMA función de área — estarían igual de mal. La única red que lo caza es **`kata-raw-nn` (Task 10)**. Por tanto: no "corregir" los flags a ojo por memoria de KataGo; si Task 10 falla en posiciones con territorio/pass-alive, los flags del área son el sospechoso #1. No tocar `fastBoard.ts` vendorizado para esto sin que Task 10 pruebe la divergencia (podría ser intencional que web-katrain reproduzca a KataGo).
