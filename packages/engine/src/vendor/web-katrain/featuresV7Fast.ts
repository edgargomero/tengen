 ...
 * Origen: src/engine/katago/featuresV7Fast.ts. Licencia completa en packages/engine/THIRD-PARTY-LICENSES.
 * Cambios de tengen y procedimiento de re-sync: docs/research/fase-engine/adaptaciones-upstream.md
 *
 * VENDORIZADO SOLO COMO ORÁCULO DE TEST (NHWC intacto). El encoder de producción es el fork NCHW
 * `encoding/featuresV7.ts`. Único cambio respecto a upstream: los imports de tipos `Player`/`GameRules`
 * (de `../../types`) y `getOpponent` (de `../../utils/gameLogic`) se resuelven a definiciones locales
 * mínimas (mismo patrón que otros archivos vendorizados); la lógica de planos/globals es verbatim.
 */

import { BLACK, WHITE, EMPTY, PASS_MOVE, BOARD_SIZE, computeLibertyMap, computeAreaMapV7KataGo, type StoneColor } from './fastBoard';

// Definiciones locales mínimas que en upstream vienen de `../../types` y `../../utils/gameLogic`.
