// Puente entre el protocolo tipado (protocol.ts) y `LocalEngine`. Archivo 100% de tengen. Dos partes:
//  1. `createWorkerHandler` — unidad PURA y testeable (sin tocar `self`): gestiona la cola serial de
//     operaciones, el streaming de `analyze` y el BYPASS de `stop`. Se testea en Node con un canal
//     mock (tests/worker.test.ts); sin factorizarla no habría cobertura del worker en la suite.
//  2. Entrada del browser (al fondo): SÓLO dentro de un dedicated worker real; crea un `LocalEngine`
//     con la factory por defecto (/models/<id>.onnx del dev server) y cablea `self`.
...
// --- Entrada del browser -------------------------------------------------------------------------
// SÓLO se ejecuta dentro de un dedicated worker real. En Node/vitest `self` es `undefined` (guarda
// primaria); el chequeo de `document` descarta además un entorno tipo jsdom (donde `self` sería la
// ventana). Así, importar este módulo en la suite NO cablea `onmessage`.
